...
var prefix string
var reconnectChn = make(chan int)
...
func main() {
	...
	if autoReload { //需要连接zk,那么就必须等获取到zk服务器信息之后才继续下面的动作
		...
		zkMgr.InitZk(MainRoutineContext, servers)
		watchIfReconnected()
	}
...
func dealFileItem(ctx *context.RoutineContext) {
	...
	for {
		...
		if success {
			...
			zkRetryer.DoRetry(CreateOrUpdateInstNode, ctx)
		}
...
func watchIfReconnected() {
	go func() {
		for {
			<-zkMgr.ReconnectWatcher
...
func watchFileNode(ctx *context.RoutineContext) {
	...
	for {
		...
		if watcher, ok := output.Result.(zkMgr.NodeWatcher); ok {
			...
			case <-w:
				glog.Infof("[Rtn%d]与ZK服务器重连成功,准备重新获取配置.", ctx.RoutineId)
				fmt.Println("重连....")
				removeWaiter(ctx.RoutineId)
				return
			}
...
type waiter chan int32

var waiterMap = make(map[int32]chan int32)
var waiterLock = &sync.Mutex{}

func createWaiter(id int32) waiter {
	waiterLock.Lock()
...
func notifyWaiter() {
	waiterLock.Lock()
	for _, w := range waiterMap {
		w <- 1
	}
	defer waiterLock.Unlock()
}
func removeWaiter(id int32) {
	waiterLock.Lock()
	defer waiterLock.Unlock()
	if _, isPresent := waiterMap[id]; isPresent {
		delete(waiterMap, id)
	}
}
...
func checkFileNode(ctx *context.RoutineContext) (bool, error) {
	...
	fmt.Println("校验玩了啦啦啦啦。。。", *output)
	if exists, ok := output.Result.(bool); ok {
		fmt.Println("校验结果是。。。", exists, "...:", output.Err)
